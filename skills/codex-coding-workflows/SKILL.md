...
2. Use `team-orchestration` when the user is explicitly asking about multi-agent or team coordination as a reusable execution pattern, or when a specialist workflow needs real subagent coordination with durable team-run state and packet-based worker dispatch.
3. Use `live-conductor` when a team run already exists and the user wants to actually execute it through live subagents in the current Codex runtime.
4. Use `policy-risk-check` only when the user is explicitly asking for a risk classification or policy-based verification recommendation.
5. Use `implementation-plan` when the user needs a decision-complete plan before coding.
6. Use `execution-task-loop` when the work is multi-step, stateful, or likely to evolve during implementation.
7. Use `project-memory-sync` when `.codex-workflows/` is missing, stale, or needs durable facts refreshed.
8. Use `workflow-state-repair` when repo-local workflow state is invalid or malformed.
9. Use `verify-change` after non-trivial work, especially before claiming a task is done.
10. Use `review-ready-summary` when the user wants a reviewer-facing artifact or asks whether work is ready for review.
11. Use `handoff-summary` when the user wants a teammate-facing status recap or handoff artifact.
12. Use `ship-readiness-audit` when the user asks whether a branch is ready for review, shipping, release, publish, or handoff.
13. If the user asks for "what should I use?" and the answer is obvious from repo state plus the request, choose for them and explain briefly.
...
- `team-orchestration` does not replace the specialist workflow that owns the domain task.
- `live-conductor` does not replace the workflow that owns scope or synthesis; it only executes an already-prepared live run.
...
- "Design a reusable multi-agent workflow for this plugin" -> `team-orchestration`
- "Run this prepared team review through live subagents" -> `live-conductor`
...
1. Clarify the operating mode: review, team orchestration, planning, active execution tracking, memory/task sync, risk review, verification, artifact generation, or ship readiness.
...
- runtime-owned automatic memory or server-backed persistent recall beyond plugin-managed local/shared memory files and promotion logs
- permission enforcement or new permission modes
- prompt-cache control or prompt-boundary tuning
- native specialist subagents with hard runtime tool boundaries beyond plugin-managed orchestration and packet-based dispatch over existing Codex subagent tools
